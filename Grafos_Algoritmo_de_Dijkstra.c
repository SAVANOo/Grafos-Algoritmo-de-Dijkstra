...
int main()
{
    ...
    liberarMemoria(); // Libera a memória alocada dinamicamente
...
void liberarMemoria()
{
    ...
    while (atual != NULL)
    {
        TListasConexoes *conexaoAtual = atual->listaConexoes;
        while (conexaoAtual != NULL)
        {
            TListasConexoes *proximaConexao = conexaoAtual->next;
            free(conexaoAtual);
            conexaoAtual = proximaConexao;
        }

        TVertice *proxVertice = atual->next;
        free(atual);
        atual = proxVertice;
    }
}
